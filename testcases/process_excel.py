...
def process_excel_data(file_path):
    ...
    try:
        ...
        # 创建两个空字典用于存储不同来源的数据
        general_accidents_dict = {}
        simple_accidents_dict = {}
        ...
        for index, row in df.iterrows():
            ...
            if str(source).strip() == '一般事故':
                if accident_id not in general_accidents_dict:
                    general_accidents_dict[accident_id] = []
                general_accidents_dict[accident_id].append(row_data)
            if str(source).strip() == '简易事故':
                if accident_id not in simple_accidents_dict:
                    simple_accidents_dict[accident_id] = []
                simple_accidents_dict[accident_id].append(row_data)
        
            if prefix is None:
                py, pm, _ = row_data['事故发生时间'].split('-')
                prefix = f"{py}_{pm}_"

        return general_accidents_dict, simple_accidents_dict, prefix
...
def write_to_file(general_accidents, simple_accidents, prefix):
    try:
        # 将数据写入json文件
        with open('./outputs/' + prefix + 'general_accidents_data.json', 'w', encoding='utf-8') as f:
            json.dump(general_accidents, f, ensure_ascii=False, indent=2)
            
        with open('./outputs/' + prefix + 'simple_accidents_data.json', 'w', encoding='utf-8') as f:
