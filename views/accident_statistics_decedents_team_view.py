...
def analyze_accidents_decedents_team_view(file_path):
    ...
    try:
        ...
        # 根据身份证明号码去重，保留第一次出现的记录
        unique_deaths = death_cases.drop_duplicates(subset=['事故编号'], keep='first')
        
        # 统计所属中队的出现次数，在统计时将数值转换为字符串
        team_counts = unique_deaths['所属中队'].apply(lambda x: str(int(x))).value_counts().to_dict()
